...
def get_reference(ref_N: int, file_dir: str):
    with open(f"{file_dir}/reference_{ref_N}.json", encoding="utf-8") as f:
...
def HDBSCAN(embeddings, metric: str = 'cosine', cluster_selection_method: str = "eom", min_cluster_size = None):
    ...
    # 최소 cluster size는 최소 2 이상이어야 합니다.
    ...
    try:
        ...
        print("HDBSCAN clustering failed:", e)
        labels = None
    return labels

class HDBSCAN_cluster:
    def __init__(self, embedding_path: str, reference_path: str):
        with open(reference_path, mode="r", encoding="utf-8") as f:
    ...
    def get_embeddingfile(self):
        unique_embeddings = {}
        # "checkpoint_"로 시작하는 JSON 파일들만 선택합니다.
        file_list = [
            os.path.join(self.embedding_path, file)
        ...
        for file in file_list:
            with open(file, mode="r", encoding="utf-8") as f:
                try:
                    ...
                    continue
            if not isinstance(jsonfile, list):
                print(f"File {file} does not contain a list. Found type: {type(jsonfile)}")
                continue
            for idx, item in enumerate(jsonfile):
                if not isinstance(item, dict):
                    print(f"In file {file}, item at index {idx} is not a dict (type: {type(item)}). Value: {item}")
                    continue
                if "ID" not in item:
                    print(f"In file {file}, item at index {idx} lacks 'ID' key. Value: {item}")
                    continue
    ...
    def process_by_date(
        ...
        if output_path is None:
            output_path = "C:/Users/tpdud/code/gogo/Database/Clusters/cluster_result.json"
