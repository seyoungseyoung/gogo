...
# -----------------------
# 실행 함수 (키워드 조합이 없는 기사 수집)
# -----------------------
def collect_articles_without_keyword_triplets(triplet_csv_path, article_folder_path, output_path):
    # '3keywords.csv' 파일을 읽어 유효한 키워드 3개 조합 집합 생성
    ...
    for year in range(2019, 2026):
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc=f"{year} 기사 필터링"):
            ...
            # 반대로, 키워드 3개 조합이 포함되지 않은 기사만 선택
            if not article_contains_keyword_triplet(body, keyword_triplets):
...
# 실행 예시
triplet_csv_path = "3keywords.csv"  # 기존 코드에서 생성한 키워드 조합 파일
article_folder_path = "/home/kororu/KoBERT"
output_path = "collected_articles_without_keyword_triplets.csv"

collect_articles_without_keyword_triplets(triplet_csv_path, article_folder_path, output_path)
