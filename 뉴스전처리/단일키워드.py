...
# 저장 디렉터리 지정 (Windows 경로)
output_dir = r'C:\Users\tpdud\code\gogo\Database\뉴스키워드전처리'
...
# CPU 처리 함수
def process_with_cpu(df, target_words, must_keep_keywords):
    results = []
...
for year in range(2019, 2026):
    ...
    results = process_with_cpu(df, target_words, must_keep_keywords)
    discard_df = df[results]
    keep_df = df[~results]
